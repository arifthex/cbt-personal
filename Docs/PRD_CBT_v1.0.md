...
- Sistem menggunakan auto partitioning berbasis range 100 exam_id untuk tabel besar seperti:
  exam_attempts
  answers
  exam_results
- PostgreSQL secara otomatis membuat partisi baru saat exam_id melewati kelipatan 100 (misal ujian ke-101, 201, dst).
- Tujuan: menjaga performa query dan mencegah table bloat untuk ujian berskala besar (ribuan siswa dan soal).
- Partisi dikelola langsung oleh trigger PL/pgSQL, tanpa perlu intervensi backend.
- Backend tidak perlu mengetahui detail partisi — tetap menggunakan query ke tabel utama.
- Admin sistem dapat memantau partisi melalui endpoint /admin/partitions (lihat API Spec update).
...
Semua tabel ujian masif (exam_attempts, answers, exam_results) menggunakan PostgreSQL native range partitioning.
Partisi dibuat otomatis lewat trigger yang mendeteksi rentang exam_id / attempt_id / exam_question_id.
Range = 100 exam per partisi (konfigurasi ini bisa diubah di BE/db/schema.sql).
- Impact on Deployment:
Tidak ada perubahan pada schema logic di aplikasi — migrasi otomatis tetap aman.
Namun, DevOps perlu memastikan bahwa user DB memiliki hak CREATE TABLE agar trigger bisa berjalan.
...
- Tambahkan cache cleanup + partition precreate job di background Golang service:
  - cron_clean_expired_cache() → membersihkan cache_stats
  - cron_precreate_partitions() → membuat 10 batch partisi berikutnya jika belum ada (opsional)
- Admin dashboard menampilkan estimasi jumlah partisi aktif di sistem.
